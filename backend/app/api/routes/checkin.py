...
def _serialize_checkin(checkin: DailyCheckin) -> CheckinRead:
    ...
    return CheckinRead(
        ...
        child_sleep_quality=float(details.get("child_sleep_quality", 6)),
        ...
        transition_difficulty=checkin.transition_difficulty,
...
def _resolve_priority_scenario(checkin: DailyCheckin, recent_scenario: str | None) -> str | None:
    ...
    if checkin.transition_difficulty >= 7 or checkin.meltdown_count >= 2:
        return "transition"
...
def _checkin_context_text(checkin: DailyCheckin) -> str:
    ...
    return (
        ...
        f"睡眠质量 {float(details.get('child_sleep_quality', 6)):g}/10，睡眠困扰：{sleep_issues}；"
        f"当前精神状态 {mood_label}，身体不适：{physical_discomforts}；"
        f"昨日过激行为：{aggressive_behaviors}；负面情绪：{negative_emotions}；"
        f"冲突/崩溃 {meltdown_label}，过渡难度 {checkin.transition_difficulty:g}/10；"
...
def _build_checkin_result(
    ...
    today_one_thing = CoachAgent().today_one_thing(signal.risk_level, recent_scenario)
    ...
    return signal, today_one_thing, action_plan
...
def create_checkin(
    ...
    checkin.transition_difficulty = payload.transition_difficulty
